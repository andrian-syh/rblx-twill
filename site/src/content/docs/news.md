...
The second is worth dwelling on. Both
[Services](/core-guides/services/) and [Architecture](/explanation/architecture/)
have described the correct behaviour since v1.0.0. The documentation was right
and the code was wrong, for ten months, because nobody read them side by side.
...
- An integration harness covering the paths the test suite cannot reach: `admit`
  ...
  replication binding, the blocked-route handling in `Navigation`, and the new
  `owner` on `Replication.Subscribe`, which only exists on the client. See
