...
to add up to anything in particular. Weights of `75` and `25` and weights of `3`
and `1` describe the same table.
...
Every function taking a `luckFactor` refuses one at or below `-1`. At `-1` the
base is zero and below it the base is negative, either of which produces a
weight that is infinite or not a number, and no comparison downstream catches
that. Bad luck is spelled with a factor between `-1` and `0`.
...
odds shown to a player cannot drift from the odds being drawn. A table
rebalanced without the display being updated is otherwise a silent, permanent
lie.

Roblox requires the possible outcomes and their numerical odds to be shown
before a player spends Robux, or currency bought with Robux, on a random result.
...
Read the [paid random items policy][policy] rather than relying on this
paragraph, since the wording is Roblox's and it changes.

[policy]: https://create.roblox.com/docs/production/monetization/paid-random-items

## Where the draw comes from

The default source is an ordinary `Random`. That is right for variety nobody
...
| :--- | :--- | :--- |
| `source` | `Source?` | Where draws come from. A fresh `Random` when left out, which is not cryptographic. |

**Returns**

`Pool<T>` - An empty pool.

Throws when given anything but a `Random`, a table carrying `NextNumber`, or
nothing.
...
| `weight` | `number` | How often it should come up against the others. Zero or more, and finite. |
...
A negative weight is refused, because a table holding one reports odds above one
for everything else.
...
Removing something that was never there does nothing rather than failing.
...
`boolean` - `true` when there is nothing held.
...
Throws when the entries together come to infinity, which an extreme exponent can
reach.

The total is cached against the luck it was worked out at, and worked out again
when an entry changes or a different luck is asked for. Reading the same pool at
the same luck twice costs one walk, not two.
...
| Name | Type | Description |
| :--- | :--- | :--- |
| `luckFactor` | `number?` | How much luck to apply. None when left out. Above `-1`. |
...
```text
Chance<3 entries, 111 weight>
```

The weight shown is the unaided total, before any luck is applied.

## What this does not do

No pity, and no guarantee after so many draws. That needs a counter per player
that survives a session, which is [`Data`](/reference/data/)'s job rather than
this module's. It is a handful of lines where the draw happens:
...
No multi-draw. Ten pulls is a loop. Drawing without replacement is `RemoveItem`
inside one.
