...
Nothing else has to be called. The same write that reaches the client updates the
player list, and a shared key updates it for everyone at once. The folder lives
on the player, so it leaves when they do.

For the trade-offs, see
[Show stats in the player list](/guides/leaderstats/).
...
| :--- | :--- | :--- |
| `Name` | `string` | The column heading, and the name of the value object. |
| `Key` | `string` | The [`Replication`](/reference/replication/) key to watch. |
| `Path` | `string?` | A path inside that key. Omit to watch the whole key. |
...
Throws on a malformed list, and on a stat name that is already bound.

:::note[Binding adds, it does not replace]
Calling this again keeps what was bound before and appends to it, so a system can
register its own stat without knowing what else exists. Binding ten stats to one
key costs one listener, not ten.
:::
...
:::caution[The player list sorts `IntValue` and `NumberValue` only]
Text is shown but never sorted. A big number therefore reads correctly and ranks
nowhere, because no value object can hold it as a number.

If ranking matters more than exactness, store a clamped plain number alongside
the big one and bind that.
:::
