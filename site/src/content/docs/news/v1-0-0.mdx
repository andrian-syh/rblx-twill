...
a path fell back to a numeric key, but writing and waking subscribers did not,
so `Subscribe("Data.Items.2")` never fired. All three now share one lookup.

* **`Serialize.FindUnstorable` did not inspect keys.** A table mixing named and
numbered keys encodes to an array with the named keys silently dropped. It is
now refused, along with gaps in a numbering and text that is not valid UTF-8.
