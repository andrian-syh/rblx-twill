...
`Filter` answers `nil` when the Roblox filter is unreachable, never the text
that went in. Returning the original would turn an outage into an unfiltered
...
player's current allowance, and all data handling live there, because an
opponent who can read the limiter can find its edge.

A client cannot request anything from `Replication`. Everything a client holds
there arrived because the server decided to send it. Where a client genuinely
has a question, `Net` answers it: a remote with reply types, metered and
screened like every other call. What is refused is the pull, not the question.
...
with two levels and a game with twelve are the same problem, and any set of
names Twill picked would be wrong for one of them.

`Admin.Configure` has no default rank for the same reason. A gate that guesses
is a gate that eventually guesses wrong, in the direction nobody notices until
it matters.
...
same write as the reward, so a redelivered receipt is recognised, and success is
reported to Roblox only once that write is confirmed. Everything else answers
...
`Compress` refuses a truncated payload rather than reading it as a smaller
value, because a save that looks complete and is not is the worst failure
available.
...
What Twill does not do is pretend the phases are sequential when they are not.
Each `Start` runs on its own thread, so a service that yields there does not
...
expensive, and the framework cannot know how often you will read. So the value
is handed over as it is, and the rule that comes with it is stated instead:
treat what you read as read only, and publish through the functions.
...
rate before parsing a command line, because parsing can reach outward to Roblox
...
Twill bundles Cmdr, a cryptography library, and others rather than
...
round-trip, which requires reattaching a metatable JSON drops and copying
results out of a shared limb pool. Neither is optional, and neither is obvious.
